...
function addNewGanttChartItem() {
    var date = ganttChartView.getProjectStart(), year = date.getFullYear(), month = date.getMonth(), day = date.getDate();
    var item = {
        content: 'New task', indentation: 0, isMilestone: false,
        start: new Date(year, month, day + 2, 8, 0, 0), finish: new Date(year, month, day + 3, 16, 0, 0),
        completedFinish: new Date(year, month, day + 2, 8, 0, 0),
        itemIndex: -1
    };
...
function scheduleChart() {
    ...
    var date = ganttChartView.getProjectStart(), year = date.getFullYear(), month = date.getMonth(), day = date.getDate();
    scheduleChartSettings.timelineStart = new Date(year, month, day - 1, 8, 0, 0);
    scheduleChartSettings.timelineFinish = new Date(year, month + 3, day + 1, 8, 0, 0);
    scheduleChartSettings.currentTime = new Date(year, month, day + 2, 10, 0, 0);
    scheduleChartSettings.displayedTime = new Date(year, month, day - 1, 8, 0, 0);
...
function loadChart() {
    ...
    var date = ganttChartView.getProjectStart(), year = date.getFullYear(), month = date.getMonth(), day = date.getDate();
    loadChartSettings.timelineStart = new Date(year, month, day - 1, 8, 0, 0);
    loadChartSettings.timelineFinish = new Date(year, month + 3, day + 1, 8, 0, 0);
    loadChartSettings.currentTime = new Date(year, month, day + 2, 10, 0, 0);
    loadChartSettings.displayedTime = new Date(year, month, day - 1, 8, 0, 0);
...
function print() {
    ...
    ganttChartView.print({ title: 'Gantt Chart (printable)', isGridVisible: true, columnIndexes: [1, 2], timelineStart: ganttChartView.timelineStart, timelineFinish: ganttChartView.timelineFinish, preparingMessage: '...' });
}